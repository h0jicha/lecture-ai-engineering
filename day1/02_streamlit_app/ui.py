...
def display_history_list(history_df):
    ...
    for i, row in paginated_df.iterrows():
        with st.expander(f"{row['timestamp']} - Q: {row['question'][:50] if row['question'] else 'N/A'}..."):
            ...
            cols = st.columns(3)
            # NaNの場合はハイフン表示
            cols[0].metric("ROUGE", f"{row['rouge_score']:.4f}" if pd.notna(row['rouge_score']) else "-")
...
def display_metrics_analysis(history_df):
    ...
    metric_options = ["bleu_score", "similarity_score", "relevance_score", "word_count", "rouge_score"]
    # 利用可能な指標のみ選択肢に含める
